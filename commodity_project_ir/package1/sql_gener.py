...
def map_dftype_to_sqldtype(df):
    ...
    data_type_map = {'i':'INTEGER', 'f':'REAL', 'None':'NULL', 'object':'TEXT'}

    df_cols = list(df.columns)
    
    df_types_list = [ data_type_map[col].dtype.kind() for col in df_cols ]
    ...
    #A reduce, a lambda, and a list comprehension all in one line....sometime I cant help but impress myself
    ...
    sql_table_arg_list = [ "{name} {type}".format(name=sql_col[i], type=sql_types_list[i]) for i in list(range(len(sql_col)))]
...
import pandas as pd; df = pd.read_csv('/Users/johnmacnamara/Desktop/test_data2.csv')
